...
export function validateCapabilityWithNativeRecovery(
  kind: CapabilityKind,
  run: StructuredRunResult,
  ...
  try {
    ...
    const recovered = recoverNativeSessionArtifacts(
      kind,
      run.workspaceRoot,
      run.nativeSessionId,
      request.outputDir,
...
function recoverNativeSessionArtifacts(
  kind: Extract<CapabilityKind, 'image' | 'video'>,
  workspaceRoot: string,
  ...
  const candidates = fs
    ...
    .filter((filePath) => {
      ...
      if (!extensions.has(extension) || !fs.lstatSync(filePath).isFile()) {
        return false;
      }
  ...
  const target = path.join(outputDir, `grok-${nativeSessionId}${extension}`);
...
function appendCapabilityAudit(logPath: string, summary: CapabilitySummary): void {
  const parsed = parseJson(fs.readFileSync(logPath, 'utf8'));
  if (!isRecord(parsed)) {
  ...
  fs.writeFileSync(logPath, `${JSON.stringify(parsed, null, 2)}\n`, { mode: 0o600 });
}

function parseIsoDateMs(value: string, label: string): number {
